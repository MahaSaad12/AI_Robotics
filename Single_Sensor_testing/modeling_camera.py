...
class PhaseCanvas(FigureCanvas):
    ...
    def plot_sensor_and_phases(self):
        sensor_positions = self.get_sensor_positions_from_csv("Camera_calibration_object_poses.csv")  # Read from CSV
        fov = 60
        ...
        # Define a smaller set of colors (this can be extended if needed)
        ...
        for sensor_idx, sensor_position in enumerate(sensor_positions):
            frustum_color = sensor_colors[sensor_idx % len(sensor_colors)]  # Wrap around the colors
        ...
        # We need to structure the lines correctly
        lines = []
        for line in connection_lines:
            x_vals = [line[0][0], line[1][0]]  # x-coordinates
            y_vals = [line[0][1], line[1][1]]  # y-coordinates
            z_vals = [line[0][2], line[1][2]]  # z-coordinates
            lines.append([x_vals, y_vals, z_vals])  # Structure the lines

        # Now create the Line3DCollection with properly structured lines
        line_collection = Line3DCollection(lines, colors=connection_colors, linewidths=1.5, alpha=0.8)

        # Add the collection to the axes
        self.ax.add_collection3d(line_collection)

        # Manually set the axis limits to make sure the plot scales correctly
        x_vals = np.array([line[0][0] for line in connection_lines] + [line[1][0] for line in connection_lines])
        y_vals = np.array([line[0][1] for line in connection_lines] + [line[1][1] for line in connection_lines])
        z_vals = np.array([line[0][2] for line in connection_lines] + [line[1][2] for line in connection_lines])

        # Use auto_scale_xyz with these values
        self.ax.auto_scale_xyz(x_vals, y_vals, z_vals)  # This ensures the plot scales correctly
    ...
    def get_sensor_positions_from_csv(self, csv_file):
        # Read sensor positions from CSV file
        df = pd.read_csv(csv_file)
        sensor_positions = df[['x', 'y', 'z']].values
