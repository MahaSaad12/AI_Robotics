...
class PhaseCanvas(FigureCanvas):
    ...
    def get_sensor_rotation(self, sensor_idx):
        """
        Computes the rotation matrix for a given sensor index.
        - X-axis rotation is fixed at 42° for all sensors.
        - Z-axis rotation increases in steps of 30° per sensor.
        - Y-axis rotation can be customized (example: sensor_idx * 10°).
        """
        rotation_x = self.rotation_matrix_x(42)
        rotation_z_angle = 30 * (sensor_idx + 1)  # Z-axis rotation: 30, 60, 90, ..., 180
        rotation_y_angle = 10 * sensor_idx  # Example: Different Y rotation for each sensor

        rotation_y = self.rotation_matrix_y(rotation_y_angle)
        rotation_z = self.rotation_matrix_z(rotation_z_angle)
    ...
    def rotation_matrix_x(self, theta):
        """Returns a 3x3 rotation matrix for rotation around the X-axis."""
        c, s = np.cos(np.radians(theta)), np.sin(np.radians(theta))
        return np.array([[1, 0, 0], [0, c, -s], [0, s, c]])

    def rotation_matrix_y(self, theta):
        """Returns a 3x3 rotation matrix for rotation around the Y-axis."""
        c, s = np.cos(np.radians(theta)), np.sin(np.radians(theta))
